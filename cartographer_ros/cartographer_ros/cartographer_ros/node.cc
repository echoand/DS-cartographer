...
bool Node::HandleTrajectoryQuery(
  ...
  std::string txt_filename = "/home/lxb/carto.txt";
  ...
  for (int i = 0; i < response.trajectory.size(); i++){
    ...
    double q_x = response.trajectory[i].pose.orientation.x; 
    double q_y = response.trajectory[i].pose.orientation.y; 
    double q_z = response.trajectory[i].pose.orientation.z; 
